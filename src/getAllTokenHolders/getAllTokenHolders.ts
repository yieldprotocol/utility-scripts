...
async function getAllTokenHolders_(provider: JsonRpcProvider, tokenAddresses: string[]): Promise<TokenHolder[]> {
    ...
    await Promise.all(tokenAddresses.map(async tokenAddress => {  
        ...
        const addresses = events.map(event => tokenContract.interface.parseLog(event as any)!.args.to); 
        ...
        await Promise.all( uniqueAddresses.map(async address => {
            ...
            +balance.toString() > 0 && tokenHolders.push({
                tokenAddress,
...
const getAllTokenHolders = async () => {
  ...
  const provider = new JsonRpcProvider('https://mainnet.infura.io/v3/d60e29d5ff1c41cf95a6ebb557ffae08');
  const tokenHolders = await getAllTokenHolders_(provider, tokenList);

  const writer = csvWriter({ headers: ['token address', 'holder address', 'holder balance']});
  writer.pipe(fs.createWriteStream('./src/getAllTokenHolders/tokenHolders.csv'));
  
  tokenHolders.forEach(tokenHolder => {
    // console.log( tokenHolder.tokenAddress, tokenHolder.holderAddress, tokenHolder.holderBalance)
    writer.write([tokenHolder.tokenAddress, tokenHolder.holderAddress, tokenHolder.holderBalance]);
  });
